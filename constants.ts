...
const SYMBOL_MAP: Record<GameTheme, Record<SymbolType, string>> = {
  NEON: {
    ...
    [SymbolType.WILD]: '/symbols/horseshoe.png', [SymbolType.SCATTER]: '🎰', ...JP_ICONS,
...
export const GET_SYMBOLS = (theme: GameTheme): Record<SymbolType, SymbolConfig> => {
  ...
  return {
    ...
    [SymbolType.SCATTER]: { type: SymbolType.SCATTER, icon: icons.SCATTER, value: 0, style: 'text-white drop-shadow-[0_0_15px_#3F51B5]', bg: imgTheme ? T : TILE_BGS.SCATTER, ...(isDragon && { imageScale: 1.2 }), ...(isPiggy ? { imageScale: 1.8 } : ((isPirate || isArctic || isCandy || isEgypt) && { imageScale: 1.5 })) },
    ...
    [SymbolType.COIN]: { type: SymbolType.COIN, icon: icons[SymbolType.COIN] ?? '/symbols/coin.png', value: 3.5, style: 'drop-shadow-[0_0_8px_rgba(250,204,21,0.9)]', bg: (isPiggy || imgTheme) ? TILE_BGS.TRANSPARENT : TILE_BGS.YELLOW, highlightClass: 'bg-yellow-500/40 shadow-[0_0_50px_rgba(234,179,8,0.9)] border-yellow-300/50', ...(isPiggy && { imageScale: 1.6 }) },
  };
};
