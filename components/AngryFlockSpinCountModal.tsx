...
// Angry Flock's bonus spins 3-6 free spins (before the bird-color wheel decides
// the wild pattern that governs them).
const SEGMENTS: Seg[] = [
    { label: '3', count: 3, color: '#b45309', weight: 30 },
    { label: '4', count: 4, color: '#dc2626', weight: 28 },
    { label: '5', count: 5, color: '#16a34a', weight: 24 },
    { label: '6', count: 6, color: '#2563eb', weight: 18 },
];
...
export const AngryFlockSpinCountModal: React.FC<Props> = ({ isOpen, onComplete }) => {
                    ...
                    <div className="relative" style={{ width: 300, height: 350 }}>
                        <div ref={tickerRef} className="absolute z-20"
                            ...
                            <svg width={28} height={44} viewBox="0 0 40 62" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M20 2C10 2 2 10 2 20C2 30 12 56 20 60C28 56 38 30 38 20C38 10 30 2 20 2Z"
                                    fill="url(#afscgrad)" stroke="#111827" strokeWidth="2.5"/>
                                <circle cx="20" cy="20" r="12" fill="#fbbf24"/>
                                <circle cx="20" cy="20" r="5" fill="#fffbeb"/>
                                <defs>
                                    <linearGradient id="afscgrad" x1="20" y1="2" x2="20" y2="60" gradientUnits="userSpaceOnUse">
                        ...
                        {phase === 'ready' && (
                            <button onClick={() => doSpinRef.current()}
                                ...
                                <span className="font-black text-[#b45309] uppercase tracking-tight" style={{ fontSize: 18, fontFamily: 'Titan One, cursive', lineHeight: 1 }}>SPIN</span>
                            </button>
                        )}
                    </div>
                </div>
                ...
                <div className="absolute inset-0 z-[210] flex items-center justify-center animate-pop-in">
                    <div className="flex flex-col items-center gap-3 rounded-3xl px-10 py-7 text-center overflow-hidden"
                        ...
                        <button onClick={() => onComplete(wonSeg.count)} className="pill-pink w-full mt-1">
                            <div className="pill-face" style={{ padding: '8px 12px', fontSize: '11px', background: 'linear-gradient(180deg,#fde68a,#b45309)' }}>Next: Bird Wheel →</div>
                        </button>
                    </div>
                </div>
            )}
