...
// Higher multiplier = rarer. This picks the ONE wild multiplier that applies for
// the entire free-spin session (any line win through a WILD gets multiplied by it).
const SEGMENTS: Seg[] = [
    { label: '2X', color: '#b45309', mult: 2, weight: 40 },
    { label: '3X', color: '#dc2626', mult: 3, weight: 28 },
    { label: '4X', color: '#7c3aed', mult: 4, weight: 20 },
    { label: '5X', color: '#f59e0b', mult: 5, weight: 12 },
];
...
function drawWheel(ctx: CanvasRenderingContext2D, angleDeg: number, bulbPhase: number) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        ctx.font = `900 46px 'Titan One', cursive`;
        ctx.fillStyle = '#fff';
    ...
    for (let b = 0; b < totalBulbs; b++) {
        ...
        if (isLit) {
            ...
            g.addColorStop(0, '#fff'); g.addColorStop(0.3, '#fed7aa'); g.addColorStop(1, '#ea580c');
            ctx.fillStyle = g;
            ctx.shadowColor = '#f97316'; ctx.shadowBlur = 18;
            ctx.strokeStyle = '#431407';
        } else {
            ctx.fillStyle = '#431407'; ctx.strokeStyle = '#1c0701';
        }
...
interface Props { isOpen: boolean; freeSpins: number; onComplete: (mult: number) => void; }
type Phase = 'prompt' | 'ready' | 'spinning' | 'done';

export const BeastRouletteModal: React.FC<Props> = ({ isOpen, freeSpins, onComplete }) => {
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="animate-pop-in flex flex-col items-center gap-2 rounded-3xl px-7 py-5 text-center overflow-hidden"
                        ...
                        <button onClick={() => setPhase('ready')} className="pill-pink w-full mt-1">
                            <div className="pill-face" style={{ padding: '8px 12px', fontSize: '11px', background: 'linear-gradient(180deg,#fb923c,#c2410c)', color: '#fff', WebkitTextStroke: '0' }}>Spin Now!</div>
                        </button>
                    </div>
                </div>
            )}
                ...
                <div className="absolute" style={{ left: '50%', top: '50%', transform: 'translate(-50%, -50%)', width: 300, height: 350 }}>
                    <div ref={tickerRef} className="absolute z-20"
                        ...
                        <svg width={28} height={44} viewBox="0 0 40 62" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M20 2C10 2 2 10 2 20C2 30 12 56 20 60C28 56 38 30 38 20C38 10 30 2 20 2Z"
                                fill="url(#brgrad)" stroke="#111827" strokeWidth="2.5"/>
                            <circle cx="20" cy="20" r="12" fill="#fb923c"/>
                            <circle cx="20" cy="20" r="5" fill="#fff7ed"/>
                            <defs>
                                <linearGradient id="brgrad" x1="20" y1="2" x2="20" y2="60" gradientUnits="userSpaceOnUse">
                    ...
                    {phase === 'ready' && (
                        <button onClick={() => doSpinRef.current()}
                            ...
                            <span className="font-black text-[#c2410c] uppercase tracking-tight" style={{ fontSize: 18, fontFamily: 'Titan One, cursive', lineHeight: 1 }}>SPIN</span>
                        </button>
                    )}
                </div>
            )}
            ...
            {phase === 'done' && wonSeg && (
                <div className="absolute inset-0 z-[210] flex items-center justify-center animate-pop-in">
                    <div className="flex flex-col items-center gap-3 rounded-3xl px-10 py-7 text-center overflow-hidden"
                        ...
                        <div className="text-pink-100/70 text-[10px] font-black uppercase tracking-[0.25em]">Wild Multiplier</div>
                        <div className="flex flex-col items-center gap-0.5">
                            <span className="font-black leading-none" style={{ fontSize: '3.5rem', color: wonSeg.color, textShadow: '0 3px 10px rgba(0,0,0,0.55), 0 0 12px rgba(255,255,255,0.3)' }}>{wonSeg.label}</span>
                            <div className="font-black text-white text-sm uppercase tracking-[0.2em]">On Every Wild Win</div>
                        </div>
                        <button onClick={() => onComplete(wonSeg.mult)} className="pill-pink w-full mt-1">
                            <div className="pill-face" style={{ padding: '8px 12px', fontSize: '11px', background: 'linear-gradient(180deg,#fb923c,#c2410c)' }}>Start Free Spins!</div>
                        </button>
                    </div>
                </div>
            )}
