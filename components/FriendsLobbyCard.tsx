...
import { canCollect } from '../services/friendsService';
...
// Arena cards, solid background (no transparency).
export const FriendsLobbyCard: React.FC<Props> = ({ friends, onOpen }) => {
    const now = Date.now();
        ...
        <button
            ...
            {collectibleCount > 0 && (
                <div className="absolute top-2 right-2 min-w-[18px] h-[18px] rounded-full flex items-center justify-center px-1 z-10"
                    ...
                    <span className="font-black text-white leading-none" style={{ fontSize: 9 }}>{collectibleCount > 99 ? '99+' : collectibleCount}</span>
                </div>
            )}
            ...
            <div className="relative mt-2 flex items-center gap-1 rounded-full px-2.5 py-1"
                style={{ background: collectibleCount > 0 ? 'rgba(74,222,128,0.22)' : 'rgba(255,255,255,0.1)', boxShadow: collectibleCount > 0 ? 'inset 0 0 0 1px rgba(74,222,128,0.5)' : undefined }}>
                <i className="ti ti-gift" style={{ fontSize: 11, color: collectibleCount > 0 ? '#4ade80' : 'rgba(255,255,255,0.6)' }} />
                <span className="font-black leading-none" style={{ fontSize: 9, color: collectibleCount > 0 ? '#4ade80' : 'rgba(255,255,255,0.6)' }}>
                    {collectibleCount > 0 ? `${collectibleCount} Ready` : 'Send Gifts'}
                </span>
            </div>
        </button>
    );
